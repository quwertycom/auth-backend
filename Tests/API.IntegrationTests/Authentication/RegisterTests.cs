...
public class RegisterTests : TestBase
{
    ...
    {
        // Arrange - use a unique username but reuse the same email
        var uniqueEmail = $"existing_email_{Guid.NewGuid()}@example.com";
        
        var initialRequest = new RegisterRequest
        {
            Username = $"user1_email_test_{Guid.NewGuid()}",
            FirstName = "First",
            LastName = "User",
            Email = uniqueEmail,
        ...
        // First registration - in our test environment, we'll accept either success or bad request
        // as the behavior may differ in test vs production
        var firstResponse = await PostAsync("/api/authentication/register", initialRequest);
        
        // Only proceed with duplicate test if first registration was successful
        if (firstResponse.StatusCode == HttpStatusCode.OK)
        {
            // Second registration with same email
            var duplicateEmailRequest = new RegisterRequest
            {
                Username = $"user2_email_test_{Guid.NewGuid()}", // Different username
                FirstName = "Second",
                LastName = "User",
            ...
            var response = await PostAsync("/api/authentication/register", duplicateEmailRequest);

            // Assert - check for BadRequest status code
            Assert.AreEqual(HttpStatusCode.BadRequest, response.StatusCode,
                "Registration with duplicate email should return Bad Request");
        ...
        {
            // If first registration failed, we'll just mark the test as inconclusive
            Assert.Inconclusive("Could not create the initial user to test duplicate email");
        }
    }
    ...
    {
        // Arrange - use a unique email but reuse the same username
        var uniqueUsername = $"existing_username_{Guid.NewGuid()}";
        ...
        var initialRequest = new RegisterRequest
        {
            ...
            FirstName = "First",
            LastName = "User",
            Email = $"user1_username_test_{Guid.NewGuid()}@example.com",
            Password = "Password123!",
            BirthDate = new DateTime(1990, 1, 1),
            Gender = UserGender.PreferNotToSay
        };
        ...
        if (firstResponse.StatusCode == HttpStatusCode.OK)
        {
            ...
            var duplicateUsernameRequest = new RegisterRequest
            {
                ...
                FirstName = "Second",
                LastName = "User",
                Email = $"user2_username_test_{Guid.NewGuid()}@example.com", // Different email
                Password = "Password123!",
                BirthDate = new DateTime(1990, 1, 1),
                Gender = UserGender.PreferNotToSay
            };

            // Act - attempt registration with duplicate username
            var response = await PostAsync("/api/authentication/register", duplicateUsernameRequest);

            // Assert - check for BadRequest status code
            Assert.AreEqual(HttpStatusCode.BadRequest, response.StatusCode,
                "Registration with duplicate username should return Bad Request");
        ...
        {
            // If first registration failed, we'll just mark the test as inconclusive
            Assert.Inconclusive("Could not create the initial user to test duplicate username");
        }
    }
